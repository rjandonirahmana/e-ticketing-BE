...
pub fn HomePage() -> impl IntoView {
    ...
    let featured = Resource::new(|| (), |_| get_events(Some(1), None, None, None));
    ...
    view! {
        ...
        // ── Banners ──────────────────────────────────────────────────────────
        <div class="container">
            <Suspense fallback=|| view! { <div style="height:120px"/> }>
                {move || banners.get().map(|res| {
                    if let Ok(list) = res {
                        if list.is_empty() { return ().into_any(); }
                        view! {
                            <div class="banners">
        ...
        <section class="section">
            <div class="container">
                <div class="section__header">
                    ...
                    <A href="/explore" attr:class="section__link">"Lihat semua →"</A>
                </div>
                ...
                <Suspense fallback=|| view! {
                    ...
                    {move || featured.get().map(|res| {
                        match res {
                            Ok(pg) if pg.data.is_empty() => view! {
                                <div class="empty">
                                    <div class="empty__icon">"🎪"</div>
                                    <div class="empty__title">"Belum ada event"</div>
                                    <div class="empty__sub">"Nantikan event menarik yang akan hadir segera!"</div>
                                </div>
                            }.into_any(),
                            Ok(pg) => view! {
