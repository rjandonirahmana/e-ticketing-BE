...
use crate::web::api::{get_event_detail, get_event_location};
                    ...
                    {move || {
                        event
                            ...
                            .map(|res| match res {
                                ...
                                Err(_) => {
                                    view! {
                                        <A href="/explore" attr:class="btn btn--ghost btn--sm">
                                            "← Kembali"
                                        </A>
                                    }
                                        .into_any()
                            ...
                            view! {
                                <div>
                                    ...
                                    <div style="background:var(--clr-surface);border:1px solid var(--clr-border);border-radius:12px;padding:1.25rem;margin-bottom:1.25rem">
                                        <h2 style="font-weight:700;font-size:1rem;margin-bottom:.5rem">
                                            {ev.name}
                                        </h2>
                                        <p style="color:var(--clr-muted);font-size:.875rem;margin-bottom:.25rem">
                                            "📍 "{full_addr.clone()}
                                        </p>
                                        {if !city.is_empty() {
                                            view! {
                                    ...
                                    // Map placeholder
                                    <div
                                        style="width:100%;height:300px;border-radius:12px;overflow:hidden;border:1px solid var(--clr-border);margin-bottom:1.25rem;background:var(--clr-surface);display:flex;align-items:center;justify-content:center"
                                        id="venue-map-mount"
                                    >
