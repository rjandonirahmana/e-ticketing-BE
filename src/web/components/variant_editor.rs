...
pub fn rows_to_json(rows: &[VariantRow], removed_ids: &[String]) -> Result<String, String> {
    if rows.is_empty() {
        return Err("Minimal satu varian tiket wajib diisi.".into());
    }
...
pub fn VariantEditor(
    ...
    view! {
        <div class="medit-section-header">
            <span class="medit-section-label">"VARIAN TIKET"</span>
        </div>
        <p style="font-size:12px;color:var(--text-muted);margin:0 0 10px">
            "Tentukan jenis tiket beserta harga & kuotanya (cth. Reguler, VIP, VVIP)."
        </p>
