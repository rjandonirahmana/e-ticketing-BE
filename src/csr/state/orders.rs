...
// ── BE response types ─────────────────────────────────────────────────────────
// Matches the new enriched GET /orders response (OrderListItem on the backend).
...
struct BeOrder {
    ...
    // Enriched event fields (from first order item's event)
    #[serde(default)]
    pub event_name: Option<String>,
    /// ISO datetime string, e.g. "2024-05-24T00:00:00Z"
    #[serde(default)]
...
pub struct Order {
    ...
    pub title: String,
    pub price: String,
    /// Formatted event date, e.g. "May 24, 2024"
...
/// Format ISO datetime "2024-05-24T..." → "May 24, 2024"
                ...
                Ok(orders) => {
                    ...
                    for o in orders {
                        ...
                        match mapped.status.as_str() {
                            ...
                            _ => {
                                // WAITING FOR PAYMENT or any pending-like status
                                a.push(mapped.clone());
                                w.push(mapped);
                            }
                        }
