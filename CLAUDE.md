...
- **Baseline `038_reviews_order_fk.sql`**: pada database yang sudah berisi data (`users` ada) dan belum pernah menjalankan satu migrasi pun lewat penjalan ini, berkas sampai batas itu hanya DICATAT tanpa dijalankan — sebagian di antaranya (mis. `022_cart_payment.sql`, yang menyebut `event_variants` sebelum 023 me-rename-nya) tidak aman diulang. Database kosong menjalankan semuanya dari nol.
- **`001a` & `001b` menambal skema yang lahir DI LUAR riwayat migrasi.** `products.slug`, `cover_url`, `detail_images`, `category`, dan seluruh tabel `notifications` dipakai hampir di setiap query tetapi tak pernah dibuat berkas mana pun — keempatnya ditambahkan dengan tangan ke database berjalan. Akibatnya database kosong berhenti di `006` (`column "category" does not exist`) lalu `026` (`detail_images`), dan "bangun ulang dari nol" mustahil. `001a` memeriksa nama tabel yang berlaku (`products` bila ada, baru `events`) karena berkas BARU tetap dicoba di database lama yang sudah melewati rename 023.
- **Kolom `schema_migrations.metode`** (`jalan` / `baseline`, NULL = baseline) yang memutuskan itu. Syarat lamanya "catatan masih kosong" terlalu rapuh: satu percobaan yang gagal di tengah meninggalkan catatan parsial, dan sejak itu garis dasar tak pernah bisa dipasang lagi — tiap start mengulang kegagalan yang sama. **Geser `BASELINE` dalam commit yang sama setiap kali kamu menjalankan migrasi lewat psql.**
- **`MIGRATE_BASELINE=<nama berkas>` menggeser batas itu per-deployment**, dan berlaku walau `schema_migrations` sudah berisi. Diperlukan karena garis dasar adalah fakta tentang SATU database, bukan konstanta program: produksi sudah dimigrasi tangan sampai 038 sementara konstantanya masih 021, sehingga 022+ diputar ulang — dan `022_cart_payment.sql` gagal karena menyebut `event_variants` yang sudah di-rename jadi `product_variants` oleh 023. Berkas idempoten tak menolong di sini; yang berubah adalah sejarahnya, bukan isi berkasnya.
- `AUTO_MIGRATE=false` mematikannya. Config comes from `.env` via dotenvy — see `.env.example`.
- **Berkas yang hanya index/kolom kini TAK PEDULI nama tabel yang berlaku.** `005`, `006`, `009`, `014`, `020`, dan FK varian di `022` mencari nama yang ada di database itu (`products` lebih dulu, baru `events`; `product_variants`, lalu `event_variants`, lalu `ticket_variants`) dan MELEWATI index yang tabelnya memang sudah dibuang 023 (`order_items`, `tickets.order_item_id`). Sebelumnya berkas-berkas itu hanya benar di satu titik sejarah dan mati dengan `relation "events"/"event_variants" does not exist` setiap kali penjalan menemukannya belum tercatat di database yang sudah melewati 023.
- **`001.sql` berhenti sendiri bila `products` sudah ada.** Tanpa penjaga itu, menjalankannya lagi di database modern tidak sekadar gagal — `CREATE TABLE IF NOT EXISTS events` MEMBUAT tabel `events` kosong di samping `products` yang asli, dan sejak itu index bisa mendarat di tabel yang salah tanpa satu pun galat. Lewat penjalan otomatis kegagalannya di-rollback, tetapi lewat `psql -f` yang dijalankan tangan tabelnya sudah telanjur commit. Trigger-nya juga kini `DROP TRIGGER IF EXISTS` dulu, karena `CREATE TRIGGER` tak punya `IF NOT EXISTS`.
- **`migration-manual/` tidak ter-embed dan harus dijalankan tangan.** Nomornya memakai aturan urut yang sama: berkas bernomor N dijalankan setelah seluruh `migration/` bernomor ≤ N. Seed data uji ada di `041`/`042` (bukan lagi `007`/`010` — nomor lama itu menyesatkan, isinya menyebut `events`/`event_variants` yang sudah di-rename 023), dan `040_products_slug_unique.sql` manual karena `CREATE INDEX CONCURRENTLY` tak boleh berada di dalam transaksi. Lihat `migration-manual/README.md`.
